...
This folder is the drop-in location for the SFX one-shot **`build-complete`** (bus: `sfx`).
...
| Filename | `build-complete.ogg` |
| -------- | -------------------- |

Drop a single audio file here named exactly **`build-complete.ogg`**. Once dropped:

- Update the SFX event def in `client/src/audio/sfxManifest.ts` to set `assetPath: '/assets/sfx/build-complete/build-complete.ogg'` (or wherever the AudioSystem looks up sfx paths — see `AudioSystem.ts`).
