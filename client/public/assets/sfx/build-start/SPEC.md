...
This folder is the drop-in location for the SFX one-shot **`build-start`** (bus: `sfx`).
...
| Filename | `build-start.ogg` |
| -------- | ----------------- |

Drop a single audio file here named exactly **`build-start.ogg`**. Once dropped:

- Update the SFX event def in `client/src/audio/sfxManifest.ts` to set `assetPath: '/assets/sfx/build-start/build-start.ogg'` (or wherever the AudioSystem looks up sfx paths — see `AudioSystem.ts`).
