...
This folder is the drop-in location for the SFX one-shot **`click`** (bus: `sfx`).
...
Drop a single audio file here named exactly **`click.ogg`**. Once dropped:

- Update the SFX event def in `client/src/audio/sfxManifest.ts` to set `assetPath: '/assets/sfx/click/click.ogg'` (or wherever the AudioSystem looks up sfx paths — see `AudioSystem.ts`).
- The synth fallback continues to play if the asset fails to load.

## Mood / intent

UI click — short, crisp, square-wave-like. ~60ms, low gain (~0.18).
