# 🔊 SFX — beacon-alert-incoming

This folder is the drop-in location for the SFX one-shot **`beacon-alert-incoming`** (bus: `sfx`).
...
| Filename | `beacon-alert-incoming.ogg` |
| -------- | --------------------------- |

Drop a single audio file here named exactly **`beacon-alert-incoming.ogg`**. Once dropped:

- Update the SFX event def in `client/src/audio/sfxManifest.ts` to set `assetPath: '/assets/sfx/beacon-alert-incoming/beacon-alert-incoming.ogg'` (or wherever the AudioSystem looks up sfx paths — see `AudioSystem.ts`).
