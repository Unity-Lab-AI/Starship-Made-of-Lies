# 🔊 SFX — colony-established

This folder is the drop-in location for the SFX one-shot **`colony-established`** (bus: `sfx`).
...
| Filename | `colony-established.ogg` |
| -------- | ------------------------ |

Drop a single audio file here named exactly **`colony-established.ogg`**. Once dropped:

- Update the SFX event def in `client/src/audio/sfxManifest.ts` to set `assetPath: '/assets/sfx/colony-established/colony-established.ogg'` (or wherever the AudioSystem looks up sfx paths — see `AudioSystem.ts`).
