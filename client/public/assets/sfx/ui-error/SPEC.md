...
This folder is the drop-in location for the SFX one-shot **`ui-error`** (bus: `sfx`).
...
| Filename | `ui-error.ogg` |
| -------- | -------------- |

Drop a single audio file here named exactly **`ui-error.ogg`**. Once dropped:

- Update the SFX event def in `client/src/audio/sfxManifest.ts` to set `assetPath: '/assets/sfx/ui-error/ui-error.ogg'` (or wherever the AudioSystem looks up sfx paths — see `AudioSystem.ts`).
