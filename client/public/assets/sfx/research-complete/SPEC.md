# 🔊 SFX — research-complete

This folder is the drop-in location for the SFX one-shot **`research-complete`** (bus: `sfx`).
...
| Filename | `research-complete.ogg` |
| -------- | ----------------------- |

Drop a single audio file here named exactly **`research-complete.ogg`**. Once dropped:

- Update the SFX event def in `client/src/audio/sfxManifest.ts` to set `assetPath: '/assets/sfx/research-complete/research-complete.ogg'` (or wherever the AudioSystem looks up sfx paths — see `AudioSystem.ts`).
