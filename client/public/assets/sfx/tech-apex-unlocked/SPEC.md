# 🔊 SFX — tech-apex-unlocked

This folder is the drop-in location for the SFX one-shot **`tech-apex-unlocked`** (bus: `music`).
...
| Filename | `tech-apex-unlocked.ogg` |
| -------- | ------------------------ |

Drop a single audio file here named exactly **`tech-apex-unlocked.ogg`**. Once dropped:

- Update the SFX event def in `client/src/audio/sfxManifest.ts` to set `assetPath: '/assets/sfx/tech-apex-unlocked/tech-apex-unlocked.ogg'` (or wherever the AudioSystem looks up sfx paths — see `AudioSystem.ts`).
...
This event routes through the **`music`** bus (`sfx` / `voice` / `music`). The AudioMixer applies the bus's user-configured volume + ducking.
