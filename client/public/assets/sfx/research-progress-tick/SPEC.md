# 🔊 SFX — research-progress-tick

This folder is the drop-in location for the SFX one-shot **`research-progress-tick`** (bus: `sfx`).
...
| Filename | `research-progress-tick.ogg` |
| -------- | ---------------------------- |

Drop a single audio file here named exactly **`research-progress-tick.ogg`**. Once dropped:

- Update the SFX event def in `client/src/audio/sfxManifest.ts` to set `assetPath: '/assets/sfx/research-progress-tick/research-progress-tick.ogg'` (or wherever the AudioSystem looks up sfx paths — see `AudioSystem.ts`).
