...
This folder is the drop-in location for the SFX one-shot **`match-defeat`** (bus: `music`).
...
| Filename | `match-defeat.ogg` |
| -------- | ------------------ |

Drop a single audio file here named exactly **`match-defeat.ogg`**. Once dropped:

- Update the SFX event def in `client/src/audio/sfxManifest.ts` to set `assetPath: '/assets/sfx/match-defeat/match-defeat.ogg'` (or wherever the AudioSystem looks up sfx paths — see `AudioSystem.ts`).
