# 🔊 SFX — beacon-alert-impact

This folder is the drop-in location for the SFX one-shot **`beacon-alert-impact`** (bus: `sfx`).
...
| Filename | `beacon-alert-impact.ogg` |
| -------- | ------------------------- |

Drop a single audio file here named exactly **`beacon-alert-impact.ogg`**. Once dropped:

- Update the SFX event def in `client/src/audio/sfxManifest.ts` to set `assetPath: '/assets/sfx/beacon-alert-impact/beacon-alert-impact.ogg'` (or wherever the AudioSystem looks up sfx paths — see `AudioSystem.ts`).
