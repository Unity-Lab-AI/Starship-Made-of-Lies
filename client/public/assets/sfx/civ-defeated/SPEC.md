...
This folder is the drop-in location for the SFX one-shot **`civ-defeated`** (bus: `sfx`).
...
| Filename | `civ-defeated.ogg` |
| -------- | ------------------ |

Drop a single audio file here named exactly **`civ-defeated.ogg`**. Once dropped:

- Update the SFX event def in `client/src/audio/sfxManifest.ts` to set `assetPath: '/assets/sfx/civ-defeated/civ-defeated.ogg'` (or wherever the AudioSystem looks up sfx paths — see `AudioSystem.ts`).
