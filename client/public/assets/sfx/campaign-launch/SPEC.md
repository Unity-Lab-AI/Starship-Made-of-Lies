...
This folder is the drop-in location for the SFX one-shot **`campaign-launch`** (bus: `voice`).
...
| Filename | `campaign-launch.ogg` |
| -------- | --------------------- |

Drop a single audio file here named exactly **`campaign-launch.ogg`**. Once dropped:

- Update the SFX event def in `client/src/audio/sfxManifest.ts` to set `assetPath: '/assets/sfx/campaign-launch/campaign-launch.ogg'` (or wherever the AudioSystem looks up sfx paths — see `AudioSystem.ts`).
...
This event routes through the **`voice`** bus (`sfx` / `voice` / `music`). The AudioMixer applies the bus's user-configured volume + ducking.
