# 🔊 SFX — colony-ship-intercepted

This folder is the drop-in location for the SFX one-shot **`colony-ship-intercepted`** (bus: `sfx`).
...
| Filename | `colony-ship-intercepted.ogg` |
| -------- | ----------------------------- |

Drop a single audio file here named exactly **`colony-ship-intercepted.ogg`**. Once dropped:

- Update the SFX event def in `client/src/audio/sfxManifest.ts` to set `assetPath: '/assets/sfx/colony-ship-intercepted/colony-ship-intercepted.ogg'` (or wherever the AudioSystem looks up sfx paths — see `AudioSystem.ts`).
