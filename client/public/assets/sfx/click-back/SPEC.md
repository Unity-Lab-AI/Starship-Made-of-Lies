...
This folder is the drop-in location for the SFX one-shot **`click-back`** (bus: `sfx`).
...
| Filename | `click-back.ogg` |
| -------- | ---------------- |

Drop a single audio file here named exactly **`click-back.ogg`**. Once dropped:

- Update the SFX event def in `client/src/audio/sfxManifest.ts` to set `assetPath: '/assets/sfx/click-back/click-back.ogg'` (or wherever the AudioSystem looks up sfx paths — see `AudioSystem.ts`).
- The synth fallback continues to play if the asset fails to load.

## Mood / intent

UI back / close / cancel click — lower-pitched than click. ~70ms.
