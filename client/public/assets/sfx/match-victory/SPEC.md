...
This folder is the drop-in location for the SFX one-shot **`match-victory`** (bus: `music`).
...
| Filename | `match-victory.ogg` |
| -------- | ------------------- |

Drop a single audio file here named exactly **`match-victory.ogg`**. Once dropped:

- Update the SFX event def in `client/src/audio/sfxManifest.ts` to set `assetPath: '/assets/sfx/match-victory/match-victory.ogg'` (or wherever the AudioSystem looks up sfx paths — see `AudioSystem.ts`).
