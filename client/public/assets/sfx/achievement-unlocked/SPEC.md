...
This folder is the drop-in location for the SFX one-shot **`achievement-unlocked`** (bus: `sfx`).
...
| Filename | `achievement-unlocked.ogg` |
| -------- | -------------------------- |

Drop a single audio file here named exactly **`achievement-unlocked.ogg`**. Once dropped:

- Update the SFX event def in `client/src/audio/sfxManifest.ts` to set `assetPath: '/assets/sfx/achievement-unlocked/achievement-unlocked.ogg'` (or wherever the AudioSystem looks up sfx paths — see `AudioSystem.ts`).
