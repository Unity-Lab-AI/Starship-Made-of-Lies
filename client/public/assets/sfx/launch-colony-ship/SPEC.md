# 🔊 SFX — launch-colony-ship

This folder is the drop-in location for the SFX one-shot **`launch-colony-ship`** (bus: `sfx`).
...
| Filename | `launch-colony-ship.ogg` |
| -------- | ------------------------ |

Drop a single audio file here named exactly **`launch-colony-ship.ogg`**. Once dropped:

- Update the SFX event def in `client/src/audio/sfxManifest.ts` to set `assetPath: '/assets/sfx/launch-colony-ship/launch-colony-ship.ogg'` (or wherever the AudioSystem looks up sfx paths — see `AudioSystem.ts`).
