# 🔊 SFX — conscript-citizens

This folder is the drop-in location for the SFX one-shot **`conscript-citizens`** (bus: `sfx`).
...
| Filename | `conscript-citizens.ogg` |
| -------- | ------------------------ |

Drop a single audio file here named exactly **`conscript-citizens.ogg`**. Once dropped:

- Update the SFX event def in `client/src/audio/sfxManifest.ts` to set `assetPath: '/assets/sfx/conscript-citizens/conscript-citizens.ogg'` (or wherever the AudioSystem looks up sfx paths — see `AudioSystem.ts`).
