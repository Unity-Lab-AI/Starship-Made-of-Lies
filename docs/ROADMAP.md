...
**PHASE 17.L.D.10 shipped 2026-05-13 (tech gating + starter economy hardening):** pre-built starter buildings on every civ home planet (6 Farms + 3 Aqueducts + 4 Homes + 1 Lumber Camp + 1 Quarry + 1 Solar Array + 2 Labs) · 30-pt research pool pre-seeded so Industrial Logistics is one tick-1 click away · starter resource amounts bumped for breathing room (planks 200→300, bricks 50→120, components 60→120, electronics 25→60, wood 80→200, stone 80→200) · sim-side tech-prereq gate added to `placeBuildingCanonical` (was missing entirely — Lumber Camp + Quarry + every tech-listed building was buildable without research) · `TECH_GATED_BUILDINGS` refactored to derive from `TECH_NODES` so Cathedral / Apartment / Reactor Fission / Reactor Fusion / Reactor Antimatter / Battery Bank / God Control are all properly gated (were absent from the hand-curated string set) · Cathedral now gated by Mass Communication, Apartment by Heavy Industry · Research Lab ungated to baseline-buildable (was 180-pt climb behind Computing) + Lab cost changed from bricks + electronics to planks + bricks so it's bootstrap-able · top toolbar shows `🔬 N pts · +M/t` research-pool counter alongside resource counters · Wiki rewrite of Mechanics + Themes + Tech tree sections (29 buildings / 6 baseline / 61 tech nodes / 19 colony ships with reference mode for full-catalog Wiki view).
...
**Still open (verified 2026-05-12):** three-mode mining discriminator (`flightKind` / `shuttleState` for oneway / shuttle-single / shuttle-multi) — mining-ship.ts currently ships shuttle-single semantics only. Full per-settlement data-model bifurcation (separate inventory / workforce / population / faction per `SettlementState`) — kept as proportional-share approximation per user. Forward-replay (re-tick from snapshot through events) — only rewind shipped; forward path deferred. Per-government themed cinematic narration — one generic copy per cinematic ships today.
