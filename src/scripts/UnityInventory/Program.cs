...
namespace IngameScript
{
    ...
    {        int padID=0;int tick=0;bool setupDone=false;bool autoOrg=true;int viewIdx=0,graphIdx=0,scrollOff=0,viewPause=0;
        ...
        void FindSiblingPBs(){bootPB=padPB=null;var pbs=new List<IMyProgrammableBlock>();GridTerminalSystem.GetBlocksOfType(pbs,b=>b.CubeGrid==Me.CubeGrid&&b!=Me);foreach(var pb in pbs){string nm=pb.CustomName;string nmU=nm.ToUpper();if(nm.Contains($"[PAD{padID}]")&&nmU.Contains("UNITY BOOT"))bootPB=pb;else if(nm.Contains($"[PAD{padID}]")&&nmU.Contains("UNITY PAD"))padPB=pb;}}
        ...
        void UpdatePadTag(){string nm=Me.CustomName;int idx=nm.IndexOf("[PAD");if(idx>=0){int end=nm.IndexOf("]",idx);if(end>idx+4){string num=nm.Substring(idx+4,end-idx-4);int id;if(int.TryParse(num,out id)&&id>0)padID=id;}}if(padID==0)padID=1;padTag=$"[PAD{padID}";Me.CustomName=$"[PAD{padID}] UNITY INVENTORY";}
        ...
        void FeedRefineries(){if(padRef.Count==0)return;var pull=GetPullable();if(pull.Count==0)return;string[] oreTypes={"Iron","Nickel","Cobalt","Silicon","Magnesium","Silver","Gold","Platinum","Uranium","Stone","Scrap"};foreach(var r in padRef){r.UseConveyorSystem=false;var rO=r.GetInventory(1);if(rO!=null&&(float)rO.CurrentVolume>(float)rO.MaxVolume*.7f)continue;var rI=r.GetInventory(0);if(rI==null)continue;int totOre=0;foreach(var x in GL(rI))if(x.Type.TypeId.EndsWith("Ore"))totOre+=(int)x.Amount;if(totOre>0)continue;foreach(string oreN in oreTypes){int need=1000;foreach(var c in pull){if(need<=0)break;var cI=c.GetInventory();if(cI==null)continue;var cL=GL(cI);for(int i=cL.Count-1;i>=0&&need>0;i--){var t=cL[i];if(!t.Type.TypeId.EndsWith("Ore")||t.Type.SubtypeId!=oreN)continue;int ad=Math.Min((int)t.Amount,need);cI.TransferItemTo(rI,i,null,true,(MyFixedPoint)ad);need-=ad;}}}}}
