...
**Location:** `Unity Missile System/src/scripts/UnityBeacon/` (part of Unity Missile System)
...
- `src/scripts/UnityBeacon.cs` - Raw script file (edit this)
- `src/scripts/UnityBeacon/Program.cs` - MDK build file (auto-wrapped from UnityBeacon.cs)

**WHEN EDITING:**
1. Edit `src/scripts/UnityBeacon.cs` directly
2. Run `tools/wrap-scripts.ps1` to sync to Program.cs
3. Build with `dotnet build src/scripts/UnityBeacon -c Debug`
...
├── tools/
│   └── wrap-scripts.ps1          # Wraps raw .cs to Program.cs
...
|-----------|----------|---------|
| **UnityPad.cs** | `src/scripts/` | Launch pad, receives beacon broadcasts on LCD 9-10 |
| **UnityMissile.cs** | `src/scripts/` | Missile guidance |
| **UnityBeacon.cs** | `src/scripts/` | Runs on miner, broadcasts status |
