...
|-----------|--------|----------|---------|
| **Launch Pad** | UnityPad.cs | `src/scripts/` | Controls missiles, displays, miner tracking |
| **Missile** | UnityMissile.cs | `src/scripts/` | Guided missile with multiple targeting modes |
| **Fleet Beacon** | UnityBeacon.cs | `src/scripts/` | Broadcasts miner status to pad |
| **Inventory** | UnityInventory.cs | `src/scripts/` | Manages storage, refineries, assemblers |
...
**WARNING:** MDK builds from `Program.cs`, NOT from `UnityBeacon.cs`! Always run `tools/wrap-scripts.ps1` before building!
