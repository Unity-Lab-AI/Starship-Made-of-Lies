...
public enum S{INIT,IDLE,PRINT,BUILD,DOCK,FUEL,READY,ARM,LAUNCH,GONE}
    ...
    S cS=S.INIT;M cM=M.MAIN;T tM=T.GPS;E env=E.UNKNOWN;int sel=0,lnchTk=0,fuelTicks=0,dockTicks=0,setScroll=0,bldScroll=0,invCargoL=0,invCargoM=0,invCargoS=0,invCargoT=0;
    bool bootDone=false,hasGrav=false,warArmed,merged,conLocked,mslFound,sentAck=false,tgtSet=false,antBC=true,counting=false,hasTlm=false,mslLnch=false,mslBO=false,abtQ=false,abtS=false,shwOut=false,printing=false,prtStopped=false,bldCmp=false,pNmd=false,isCreative=false,isCtl=false,svAct=false,aAtk=false,aRS=false;
    float lcdW=512,lcdH=512,lcdS=1,lcdYS=1,batPct,h2Pct,o2Pct,icePct,ammoPct,mslBatCur=0,mslBatMax=0,mslH2Cur=0,mslH2Max=0,mslO2Cur=0,mslO2Max=0,mslIceCur=0,mslIceMax=0,prtSpeed=0.5f,prtHPos=0,prtHStep=0.2f,prtHMax=7.2f,prtVMax=10f,prtVZero=1.4f,prtVSpeed=0.5f,prtHSpeed=0.3f,prtLastVPos=0,padBatPct,padH2Pct,padO2Pct,padCargoPct,padPowerOut=0,padPowerMax=0,invCargoPct=0;
    int viewLCD=0,graphTimeIdx=0,thrAtmo=0,thrH2=0,thrIon=0,wpIdx=0,antIdx=0,clbDst=500,detDist=50,cntDn=10,sensorRng=50,lidarRng=500,lidarAng=5,reDst=500,fltMd=2,tgtRel=0,tlmTO=1000,fIdx=0,prtState=0,prtST=0,tick=0,bldNum=0,padID=0,padMedCount=0,padSurvCount=0,padCryoCount=0,pIceC=0,pUrnC=0,pH2B=0,pO2B=0,h2Target=20,o2Target=20,iceTarget=1000,uranTarget=50,h2PT=90,o2PT=90,h2Queued=0,o2Queued=0,prtBuildable,prtMissing,ctrlSel=0,ctrlPadSel=0,svInt=3,salvoIdx=0,cSpd=50,cPat=0,tSC=0,sRQ=0,lastIntTick=0,nextGridX=0,nextGridZ=0,satLaunchGridX=0,satLaunchGridZ=0;int[] lcdScroll=new int[9];
    string[] graphLabels={"30m","1h","6h","12h","1d","ALL"};
    Color cPri=new Color(0,180,255),cOK=new Color(0,255,100),cBg=new Color(10,10,15);Color cSec=new Color(100,100,100),cAcc=new Color(255,200,0),cWrn=new Color(255,180,0),cErr=new Color(255,60,60),cBdr=new Color(40,40,50),cTxt=new Color(220,220,220);float gravStr=0;bool inAtmo=false,setupDone=false,mergePaused=false;    IMyShipMergeBlock padMerge,mslMerge;
    IMyShipConnector padCon,mslConFuel,mslConAmmo,padCon1,padCon2;
    IMyTextPanel lcd1,lcd2,lcd3,lcd7,lcd8;
    List<IMyTextPanel> bbLCDs=new List<IMyTextPanel>(),mmLCDs=new List<IMyTextPanel>();
    string bbLog="",gpsData="",lastBBPhase="",mslGSt="",tgtAntenna="",tgtName="",bcTag="UNITY_MSL",mslPhase="",lKPh="",mslOutcome="",finalPhase="",padTag="[PAD1",pCTag="UNITY_PAD_CMD",pStTag="UNITY_PAD_STATUS",bcnTag="MINER_BEACON";S lastBBState=S.INIT;
    double mslAlt=0,mslSpeed=0,mslFuelPct=0,mslDTT=0,lKDst=0,mslGrav=0,mslDFP=0,mslVel=0,fnlDTT=0,satGridSpacing=5000;
    IMyButtonPanel btn;
    IMyProgrammableBlock mslPB,bootPB,invPB,signalPB;
    List<IMyBatteryBlock> mslBat=new List<IMyBatteryBlock>(),padBat=new List<IMyBatteryBlock>();
    List<IMyGasTank> mslH2=new List<IMyGasTank>(),mslO2=new List<IMyGasTank>(),padH2=new List<IMyGasTank>(),padO2=new List<IMyGasTank>();
    List<IMyWarhead> mslWar=new List<IMyWarhead>();
    List<IMyThrust> mslThr=new List<IMyThrust>(),padThr=new List<IMyThrust>();
    List<IMyGasGenerator> mslGen=new List<IMyGasGenerator>(),padGen=new List<IMyGasGenerator>();
    List<IMyReactor> mslReact=new List<IMyReactor>(),padReact=new List<IMyReactor>();
    List<IMyGyro> mslGyr=new List<IMyGyro>(),padGyr=new List<IMyGyro>();
    List<IMySensorBlock> mslSen=new List<IMySensorBlock>(),padSen=new List<IMySensorBlock>();
    List<IMyCameraBlock> mslCam=new List<IMyCameraBlock>(),padCam=new List<IMyCameraBlock>();
    List<IMyRadioAntenna> mslAnt=new List<IMyRadioAntenna>(),padAnt=new List<IMyRadioAntenna>();
    List<IMyLaserAntenna> mslLsr=new List<IMyLaserAntenna>(),padLsr=new List<IMyLaserAntenna>();
    List<IMyCockpit> mslCock=new List<IMyCockpit>();
    List<IMyLightingBlock> mslLights=new List<IMyLightingBlock>(),padLts=new List<IMyLightingBlock>();
    IMyRemoteControl mslRC;
    DateTime lMnuT,lnchT,armTime,lTlm,lTlmT,abtT,outT,lastSalvo,lSatC;
    Vector3D tgtGPS=new Vector3D(0,0,0),mslPos,lastMslPos,satLaunchTgt=Vector3D.Zero;
    List<MyWaypointInfo> wpts=new List<MyWaypointInfo>(),customWP=new List<MyWaypointInfo>();
    List<string> detAnts=new List<string>();
    IMyBroadcastListener mslL,relL,pCmdL,pStL,enmL,bcnL;
    float[] fDist=new float[16],fAlt=new float[16],fSpd=new float[16];
    const int SET_VISIBLE=7;    const string BP="MyObjectBuilder_BlueprintDefinition/",OB="MyObjectBuilder_";
    ...
    List<int> kPads=new List<int>(),kSats=new List<int>();
    Dictionary<int,string> pStat=new Dictionary<int,string>(),pErr=new Dictionary<int,string>(),satStatus=new Dictionary<int,string>();
    Dictionary<int,bool> pArm=new Dictionary<int,bool>(),pRdy=new Dictionary<int,bool>(),pMslF=new Dictionary<int,bool>(),pPrt=new Dictionary<int,bool>();
    Dictionary<int,Vector3D> pTgts=new Dictionary<int,Vector3D>(),sPos=new Dictionary<int,Vector3D>();
    List<Vector3D> dTgt=new List<Vector3D>();
    Dictionary<int,int> sBat=new Dictionary<int,int>(),satH2=new Dictionary<int,int>(),satGridX=new Dictionary<int,int>(),satGridZ=new Dictionary<int,int>();
    Queue<Vector3D> satReplaceQueue=new Queue<Vector3D>();
    Queue<int> satReplaceGridX=new Queue<int>(),satReplaceGridZ=new Queue<int>();
    List<IMyShipConnector> oreC=new List<IMyShipConnector>();
    Dictionary<long,MinerData> trkM=new Dictionary<long,MinerData>();
    class MinerData{public string name,status,cargoRaw="";public float bat,crg,h2;public Vector3D pos;public double spd,alt,dist,outboundSecs,returnSecs,etaSecs;public int drills,drillsOn,grinders,grindersOn,portNum,cycles;public bool docked,outbound;public DateTime lastSeen;public Dictionary<string,int> cargoItems=new Dictionary<string,int>();}
    Dictionary<string,int> oStk=new Dictionary<string,int>(),cStk=new Dictionary<string,int>(),cNd=new Dictionary<string,int>(),cMis=new Dictionary<string,int>(),bpNd=new Dictionary<string,int>(),bpMis=new Dictionary<string,int>();
    int ammoStock=0,ammoTarget=0,ammoQueued=0,mslAmmo=0,ammoLoad=10106,ammoEject=10106,ammoTypeIdx=0,toolTarget=10,pAmmoTarget=100,mslAmmoTarget=50000;
    string[] ammoNames={"S-10 Pistol","MR-20 Rifle","MR-50A Rifle","200mm Missile","25x184mm NATO","Autocannon","Assault Cannon","Artillery","Small Railgun","Large Railgun"},ammoBPNames={"Position0010_SemiAutoPistolMagazine","Position0040_AutomaticRifleGun_Mag_20rd","Position0050_RapidFireAutomaticRifleGun_Mag_50rd","Position0100_Missile200mm","Position0080_NATO_25x184mmMagazine","Position0090_AutocannonClip","Position0110_MediumCalibreAmmo","Position0120_LargeCalibreAmmo","Position0130_SmallRailgunAmmo","Position0140_LargeRailgunAmmo"},ammoITNames={"SemiAutoPistolMagazine","AutomaticRifleGun_Mag_20rd","RapidFireAutomaticRifleGun_Mag_50rd","Missile200mm","NATO_25x184mm","AutocannonClip","MediumCalibreAmmo","LargeCalibreAmmo","SmallRailgunAmmo","LargeRailgunAmmo"};
    ...
    int maxSel=cM==M.MAIN?5:cM==M.TGT?2:cM==M.SET?31:cM==M.WIZARD?3:cM==M.VIEW?7:0;
    ...
    if(padID==0)padID=1;
    string prtTag=$"[PAD{padID}-PRINT]";
    var allBlks=new List<IMyTerminalBlock>();
    GridTerminalSystem.GetBlocksOfType(allBlks,b=>b.CustomName.Contains(prtTag));
    ...
    if(!mslFound){
    ST(f,256,60,"NO MISSILE DETECTED",cWrn,0.7f,TextAlignment.CENTER);
    SD(f,110);
    ...
    if(mslPB!=null&&prtProj!=null&&prtProj.RemainingBlocks==0){ST(f,256,85,"Reload Programmable Block!",cWrn,0.55f,TextAlignment.CENTER);}
    ...
    ST(f,20,100,$"Gravity: {gravStr:F2} m/sÂ²",cTxt,0.6f);
