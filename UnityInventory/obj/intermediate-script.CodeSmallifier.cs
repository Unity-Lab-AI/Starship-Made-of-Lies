...
{
int padID=0,tick=0,viewIdx=0,graphIdx=0,scrollOff=0,viewPause=0;bool setupDone=false,autoOrg=true;    bool bootDone=false,sentAck=false;
    ...
    class MinerData{public string name,status,cargoRaw="";public float bat,crg,h2,o2;public Vector3D pos;public double spd,alt,dist,outboundSecs,returnSecs,etaSecs;public int drills,drillsOn,grinders,grindersOn,portNum,cycles,ice,uranium,o2Tanks,genCount,reactCount;public bool docked,outbound;public DateTime lastSeen;public Dictionary<string,int> cargoItems=new Dictionary<string,int>();}
    Color cPri=new Color(0,180,255),cSec=new Color(100,100,100),cAcc=new Color(255,200,0),cOK=new Color(0,255,100),cWrn=new Color(255,180,0),cErr=new Color(255,60,60),cBg=new Color(10,10,15),cBdr=new Color(40,40,50),cTxt=new Color(220,220,220);
    List<float> pwrHist=new List<float>(),h2Hist=new List<float>(),o2Hist=new List<float>(),cargoHist=new List<float>(),refHist=new List<float>(),asmHist=new List<float>(),prodHist=new List<float>(),pwrInHist=new List<float>(),pwrOutHist=new List<float>(),solarHist=new List<float>(),windHist=new List<float>(),reactHist=new List<float>(),genHist=new List<float>();
    ...
    List<IMyCargoContainer> subgridCargo=new List<IMyCargoContainer>(),padCargo=new List<IMyCargoContainer>(),padCargoL=new List<IMyCargoContainer>(),padCargoM=new List<IMyCargoContainer>(),padCargoS=new List<IMyCargoContainer>(),sharedCargo=new List<IMyCargoContainer>();
    ...
    Dictionary<string,int> oStk=new Dictionary<string,int>(),cStk=new Dictionary<string,int>(),iStk=new Dictionary<string,int>(),iNd=new Dictionary<string,int>(),cNd=new Dictionary<string,int>(),cMis=new Dictionary<string,int>(),cQd=new Dictionary<string,int>(),bpNd=new Dictionary<string,int>(),oNd=new Dictionary<string,int>(),aIN=new Dictionary<string,int>(),pAmmoStk=new Dictionary<string,int>(),pAmmoQ=new Dictionary<string,int>(),tStk=new Dictionary<string,int>(),tQ=new Dictionary<string,int>(),tNd=new Dictionary<string,int>(),tMis=new Dictionary<string,int>(),paNd=new Dictionary<string,int>(),paMis=new Dictionary<string,int>(),bNd=new Dictionary<string,int>(),bStk=new Dictionary<string,int>(),bQd=new Dictionary<string,int>(),bMis=new Dictionary<string,int>(),foodStk=new Dictionary<string,int>(),miscStk=new Dictionary<string,int>(),taStk=new Dictionary<string,int>(),taNd=new Dictionary<string,int>(),taMis=new Dictionary<string,int>(),taQ=new Dictionary<string,int>();
    Dictionary<string,MyDefinitionId> tBPx=new Dictionary<string,MyDefinitionId>(),paBPx=new Dictionary<string,MyDefinitionId>(),bBPx=new Dictionary<string,MyDefinitionId>(),taBPx=new Dictionary<string,MyDefinitionId>(),compBP=new Dictionary<string,MyDefinitionId>{
    ...
    int taTarget=500,ammoStock=0,ammoTarget=500,ammoQueued=0,ammoLoad=10106,ammoTypeIdx=0,pH2B=0,pO2B=0,h2Target=20,o2Target=20,h2Queued=0,o2Queued=0,pIceStg=0,pIceGen=0,pUrnStg=0,pUrnReact=0,iceTarget=0,uranTarget=0,toolTarget=20,mslAmmoTarget=50000,padMedCount=0,padSurvCount=0,padCryoCount=0,ctrlPads=0,ctrlArmed=0,ctrlReady=0,mslIce=0,mslUran=0,mslAmmo=0,mslAmmoLoad=0,warCount=0,mslGenCnt=0,mslH2Cnt=0,mslO2Cnt=0,mslReactCnt=0,mslLsrCnt=0,mslLsrLnk=0,mslAntCnt=0;
    bool ammoReq=false,warArmed=false,conLocked=false,mergeConn=false;int ammoReqType=0,ammoReqNeed=0,mslConCnt=0,ammoPushed=0,mslCount=0,mslArmed=0,mslReady=0,prtState=0,prtRem=0,prtTot=0,prtBld=0;
    IMyShipConnector padCon=null;
    HashSet<string> itemRecycle=new HashSet<string>();
    float padCargoPct=0,padBatPct=0,padH2Pct=0,padO2Pct=0,mslBatPct=0,mslH2Pct=0,mslO2Pct=0,mslH2Fill=0,mslH2Cap=0,mslO2Fill=0,mslO2Cap=0,mslBatC=0,mslBatM=0;
    string padMode="NORMAL",mslPhase="IDLE",mslTarget="---",prtPhase="CHECKING";
float mslDist=0,mslSpeed=0,mslAlt=0,mslFuel=0,mslETA=0,prtPist=0;string ctrlMode="GPS",ctrlTarget="---",ctrlStatus="IDLE";
bool printing=false;
    const string BP="MyObjectBuilder_BlueprintDefinition/",OB="MyObjectBuilder_";
    string[] ammoNames={"S-10 Pistol","MR-20 Rifle","MR-50A Rifle","200mm Missile","25x184mm NATO","Autocannon","Assault Cannon","Artillery","Small Railgun","Large Railgun"},ammoBPNames={"Position0010_SemiAutoPistolMagazine","Position0040_AutomaticRifleGun_Mag_20rd","Position0050_RapidFireAutomaticRifleGun_Mag_50rd","Position0100_Missile200mm","Position0080_NATO_25x184mmMagazine","Position0090_AutocannonClip","Position0110_MediumCalibreAmmo","Position0120_LargeCalibreAmmo","Position0130_SmallRailgunAmmo","Position0140_LargeRailgunAmmo"},ammoITNames={"SemiAutoPistolMagazine","AutomaticRifleGun_Mag_20rd","RapidFireAutomaticRifleGun_Mag_50rd","Missile200mm","NATO_25x184mm","AutocannonClip","MediumCalibreAmmo","LargeCalibreAmmo","SmallRailgunAmmo","LargeRailgunAmmo"},tlsNames={"Drill","Welder","Grinder","Rifle","Pistol","Launcher","Flare"},pAmmoBP={"AutomaticRifleGun_Mag_20rd","PreciseAutomaticRifleGun_Mag_5rd","RapidFireAutomaticRifleGun_Mag_50rd","UltimateAutomaticRifleGun_Mag_30rd","SemiAutoPistolMagazine","FullAutoPistolMagazine","ElitePistolMagazine","Missile200mm","FlareClip"},pAmmoIT={"AutomaticRifleGun_Mag_20rd","PreciseAutomaticRifleGun_Mag_5rd","RapidFireAutomaticRifleGun_Mag_50rd","UltimateAutomaticRifleGun_Mag_30rd","SemiAutoPistolMagazine","FullAutoPistolMagazine","ElitePistolMagazine","Missile200mm","FlareClip"};
    MyDefinitionId ammoBP;
    MyItemType ammoType,h2BottleType=MyItemType.Parse(OB+"GasContainerObject/HydrogenBottle"),o2BottleType=MyItemType.Parse(OB+"OxygenContainerObject/OxygenBottle");
