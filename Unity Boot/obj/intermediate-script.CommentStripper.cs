...
{
int padID=1,tick=0,bootStep=0,bootTicks=0,bootErrTicks=0,bootCompleteTicks=0,ackWaitTicks=0,extDoorsOpen=0,ventCount=0,maxAwait=18;    bool bootDone=false,waitingForAck=false,padRequested=false,padResponded=false,invRequested=false,invResponded=false,signalRequested=false,signalResponded=false,padTimedOut=false,invTimedOut=false,signalTimedOut=false;string bootError="",bootStatus="Initializing...",minerNames="";    int bootSiblings=0,minerCount=0,signalCamC=0,signalLcdC=0,doorSetCount=0,padMergeC=0,padConC=0,padBatC=0,padH2C=0,padO2C=0,padPrtC=0,invCargoC=0,invRefC=0,invAsmC=0,invGenC=0,invH2C=0,invO2C=0,awaitTicks=0;bool preBootDone=false,padReady=false,invReady=false,signalReady=false,padAck=false,invAck=false,beaconOptional=true,doorsLocked=false;    IMyBroadcastListener ackL,setupL,beaconL,bootRspL;
    Dictionary<long,string> bootMiners=new Dictionary<long,string>();
    string[] noMinerErrs={"Miners AWOL","No beacons found","Fleet ghosted you"},sysChecks={"Initializing Core","Scanning Grid","Button Panel","Detecting LCDs","IGC Channels","Request Pad Status","Await Pad Response","Missile Merge Block","Validate Pad Power","Validate Pad Fuel","Request Inv Status","Await Inv Response","Validate Inv Cargo","Validate Inv Refinery","Validate Inv Assembler","Validate Inv Gas","Request Signal Status","Await Signal Response","Validate Signal","Scan Door Systems","Verify Pressurization","Cross-Validate","Module Sync","Write Config","Beacon Detection","Controller Modules","System Ready","All Systems Operational"};
    float lcdW=512,lcdH=512,lcdS=1;
    string padTag="[PAD1",padRspData="",invRspData="",signalRspData="";
    IMyButtonPanel btn;
    ...
    List<IMyTextSurface> camLCDs=new List<IMyTextSurface>(),signalLCDs=new List<IMyTextSurface>();
    Color cPri=new Color(0,180,255),cOK=new Color(0,255,100),cBg=new Color(10,10,15);Color cSec=new Color(100,100,100),cAcc=new Color(255,200,0),cWrn=new Color(255,180,0),cErr=new Color(255,60,60),cBdr=new Color(40,40,50),cTxt=new Color(220,220,220);float overallPressure=0;    List<IMyAirVent> airVents=new List<IMyAirVent>();
