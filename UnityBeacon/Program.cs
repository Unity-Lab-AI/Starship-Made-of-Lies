...
namespace IngameScript
{
    ...
    {        string bcTag="MINER_BEACON";
        ...
        void SLB(MySpriteDrawFrame f,float x,float y,float w,float h,string lbl,float pct,Color fg,Color bg){float sx=x*lcdS,sy=y*lcdYS,sw=w*lcdS;f.Add(new MySprite(SpriteType.TEXT,lbl,new Vector2(sx,sy-2*lcdYS),null,cTxt,"Monospace",TextAlignment.LEFT,0.45f*fntS));SB(f,x+80,y,w,h,pct,fg,bg);f.Add(new MySprite(SpriteType.TEXT,$"{pct*100:0}%",new Vector2(sx+sw+90*lcdS,sy-2*lcdYS),null,fg,"Monospace",TextAlignment.LEFT,0.45f*fntS));}
